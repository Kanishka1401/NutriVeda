...
// Constant for daily calorie adjustments based on activity level
...
class DietChartController {
  ...
  static async generateDietChart(req, res) {
    try {
      ...
      const activityMultiplier = ACTIVITY_MULTIPLIERS[patient.activityLevel?.toLowerCase()] || ACTIVITY_MULTIPLIERS.moderate;
      const targetCalories = Math.round(patient.bmr * activityMultiplier);

      // Initialize Gemini AI using the notebook's proven approach
      const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
      const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });

      // Enhanced prompt structure from the proven notebook implementation
...
- Target Calories: ${targetCalories} cal/day (based on ${patient.activityLevel || 'moderate'} activity)
