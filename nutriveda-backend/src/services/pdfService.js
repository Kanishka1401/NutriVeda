...
Generate complete meal plans for all 7 days with specific foods and calories.`;
  ...
  static addHeader(doc) {
    // Add NutriVeda Logo
    const logoPath = path.join(__dirname, '../assets/nutriveda-logo.png');
    ...
    doc.fontSize(28)
        .fillColor('#059669')
        .text('Ayurvedic Diet Plan', 140, 50, { align: 'center' });
  ...
  static addFooter(doc) {
    ...
    for (let i = 0; i < pages.count; i++) {
      ...
      const logoPath = path.join(__dirname, '../assets/nutriveda-logo.png');
      if (fs.existsSync(logoPath)) {
        doc.image(logoPath, 50, 775, { width: 20 });
      }

      // Footer text and page numbers
      doc.fontSize(8)
         .fillColor('#6b7280')
         .text('NutriVeda - Personalized Ayurvedic Nutrition', 80, 780)
         .text(`Page ${i + 1} of ${pages.count}`, 480, 780);
    }
  }
